...
export default function DualAxisLineChartRenderer({ rows, cfg, rtl, measureFmt, measure2Fmt, plotW, plotH, broadcasts, onClickPoint }: ChartRendererProps) {
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <ComposedChart data={rows} margin={chartMargin(rtl, { top: 4, right: 8, bottom: 20, left: 0 })}
        ...
        <Line yAxisId="left" type="monotone" dataKey="value" name={seriesName(cfg)} stroke="var(--accent)" strokeWidth={2} strokeDasharray={seriesDash(0, cfg.series_patterns)} dot={{ r: 3 }}>
          {labels1 && <LabelList {...labels1} />}
        </Line>
        {hasSecond && (<Line yAxisId="right" type="monotone" dataKey="value2" name={cfg.measure2 ?? 'value2'} stroke="#a78bfa" strokeWidth={2} strokeDasharray={seriesDash(1, cfg.series_patterns)} dot={{ r: 3 }}>
