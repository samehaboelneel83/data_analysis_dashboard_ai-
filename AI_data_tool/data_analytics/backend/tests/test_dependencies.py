...
def _req(method="GET", headers=(), cookie=None):
    """A bare request: these tests pass the header token directly."""
...
async def test_valid_token_resolves_to_the_user_with_role_and_org_loaded(db_session):
    ...
    resolved = await get_current_user(_req(), token=token, db=db_session)
...
async def test_garbage_token_raises_401(db_session):
    with pytest.raises(HTTPException) as exc_info:
        await get_current_user(_req(), token="not-a-real-token", db=db_session)
...
async def test_token_for_a_deleted_user_raises_401(db_session):
    ...
    with pytest.raises(HTTPException) as exc_info:
        await get_current_user(_req(), token=token, db=db_session)
    assert exc_info.value.status_code == 401
